	...
	const struct amba_id *id)
{
	...
	// add by jack.
	printk("WL---> designer ID = 0x%02x\n", host->hw_designer);
	printk("WL---> revision = 0x%01x\n", host->hw_revision);
	...
	if (host->mclk > variant->f_max) {
		...
		dev_dbg(mmc_dev(mmc), "eventual mclk rate: %u Hz\n",
			host->mclk);
		printk("WL---> eventual mclk rate: %u Hz\n", host->mclk);
	}
	...
	printk("WL---> emmci initialized, done.\n");
	return 0;
