...
static u32 cst66xx_check_esd(void)
{
    ...
    HYN_INFO("ESD data:%d,0x%04x,0x%04x", flag, esd_value, hyn_66xxdata->esd_last_value);
    if (flag == 0){
        hyn_power_source_ctrl(hyn_66xxdata, 0);
        mdelay(2);
        hyn_power_source_ctrl(hyn_66xxdata, 1);
        mdelay(2);
        cst66xx_rst();
        msleep(40);
        esd_value = 0;
        hyn_66xxdata->esd_last_value = 0;
        HYN_INFO("esd_check power reset ic");
    }
    if ((hyn_66xxdata->work_mode == GESTURE_MODE)&&(esd_value != 0xA0)){
        ok = hyn_wr_reg(hyn_66xxdata,0xD0000C01,4,0,0);
        if (ok == FALSE){
            HYN_ERROR("enter_sleep failed");
        }  
    }
    
    hyn_66xxdata->esd_last_value = esd_value;
    return esd_value;
}
